...
import { ArrowLeft, Edit, Trash2, Clock, DollarSign, Award, Users, TrendingUp, BookOpen } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-2xl font-bold">{assessment.title}</h1><p className="text-gray-500">{assessment.description}</p></div>
        </div>
        <div className="flex gap-2">
          <Button asChild variant="outline"><Link href={`/dashboard/admin/assessments/${id}/edit`}><Edit className="mr-2 h-4 w-4" />Edit</Link></Button>
      ...
      <Card className="p-6"><h2 className="text-lg font-semibold mb-4">Assessment Details</h2><div className="space-y-3"><div className="flex"><span className="w-32 text-gray-500">Skill:</span><span className="font-medium">{assessment.skillName}</span></div><div className="flex"><span className="w-32 text-gray-500">Level:</span><span className="font-medium capitalize">{assessment.level}</span></div><div className="flex"><span className="w-32 text-gray-500">Tags:</span><div className="flex flex-wrap gap-1">{assessment.tags?.map((tag: string, i: number) => (<Badge key={i} variant="secondary">{tag}</Badge>)) || '-'}</div></div></div></Card>

      <Card className="p-6"><h2 className="text-lg font-semibold mb-4">Statistics</h2><div className="grid grid-cols-3 gap-4 text-center"><div><Users className="h-5 w-5 text-gray-400 mx-auto mb-2" /><p className="text-2xl font-bold">{formatNumber(assessment.totalAttempts || 0)}</p><p className="text-sm text-gray-500">Total Attempts</p></div><div><TrendingUp className="h-5 w-5 text-gray-400 mx-auto mb-2" /><p className="text-2xl font-bold">{assessment.passRate || 0}%</p><p className="text-sm text-gray-500">Pass Rate</p></div><div><Award className="h-5 w-5 text-gray-400 mx-auto mb-2" /><p className="text-2xl font-bold">{assessment.averageScore || 0}%</p><p className="text-sm text-gray-500">Avg. Score</p></div></div></Card>

      {assessment.badges && assessment.badges.length > 0 && (<Card className="p-6"><h2 className="text-lg font-semibold mb-4">Badges</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{assessment.badges.map((badge: any, idx: number) => (<div key={idx} className="flex items-center gap-3 p-3 border rounded-lg"><div className="p-2 bg-yellow-100 rounded-full"><Award className="h-5 w-5 text-yellow-600" /></div><div><p className="font-medium">{badge.name}</p><p className="text-sm text-gray-500">Score {badge.requiredScore}%+</p></div></div>))}</div></Card>)}
    </div>
  );
}
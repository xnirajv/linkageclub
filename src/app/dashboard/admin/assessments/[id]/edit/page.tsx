...
export default function EditAssessmentPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    price: '', duration: '', passingScore: '70', tags: '', isActive: true,
  });
      ...
      {currentStep === 1 && (
        <Card className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2"><input type="checkbox" id="isActive" checked={formData.isActive} onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} className="h-4 w-4" /><label htmlFor="isActive" className="text-sm">Active (visible to students)</label></div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div><label className="block text-sm font-medium mb-2">Options</label>
                  {question.options.map((option, optIndex) => (
                    <div key={optIndex} className="flex gap-2 mb-2"><Input value={option} onChange={(e) => handleOptionChange(question.id, optIndex, e.target.value)} placeholder={`Option ${optIndex + 1}`} />{question.options.length > 2 && <Button variant="ghost" size="icon" onClick={() => handleRemoveOption(question.id, optIndex)}><X className="h-4 w-4" /></Button>}</div>
                  ))}

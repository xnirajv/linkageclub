...
import { ApplicationsList } from '@/components/applications/ApplicationList'; // ✅ Fixed import path
...
export default function MyApplicationsPage() {
  ...
  const shortlistedApps = useMemo(() => 
    applications.filter((app: Application) => app.status === 'shortlisted'),
    [applications]
  );
      ...
      <div className="space-y-6 p-6">
        ...
        <div>
          ...
          <p className="text-charcoal-600 dark:text-charcoal-400">
            Track your project and job applications
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="p-4 bg-purple-50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-purple-700">{shortlistedApps.length}</p>
          </div>
          <div className="p-4 bg-green-50 rounded-lg">
            <p className="text-sm text-green-600">Accepted</p>
            <p className="text-2xl font-bold text-green-700">{acceptedApps.length}</p>
          </div>
          <div className="p-4 bg-red-50 rounded-lg">
            <p className="text-sm text-red-600">Rejected</p>
            <p className="text-2xl font-bold text-red-700">{rejectedApps.length}</p>
          </div>
        </div>

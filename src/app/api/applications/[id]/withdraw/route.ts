...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (application.applicantId.toString() !== session.user.id) {
      return NextResponse.json({ error: 'Only applicant can withdraw' }, { status: 401 });
    }

    if (application.status !== 'pending') {
      return NextResponse.json({ error: `Cannot withdraw ${application.status} application` }, { status: 400 });
    }

...
export function ShareButtons({ assessmentId, assessmentTitle, score }: ShareButtonsProps) {
  ...
  const handleDownloadCertificate = async () => {
    try {
      const response = await fetch(`/api/assessments/${assessmentId}/certificate`);
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `certificate_${assessmentTitle.replace(/\s/g, '_')}.pdf`;

    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          setIsRunning(false);
          if (autoSubmit && onTimeUp) onTimeUp();
          return 0;
        }
    ...
    <Card className="p-4">
      <div className="flex flex-col items-center">
        <div className="relative">
          <Clock className="absolute -left-6 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
          <span className={cn('font-mono font-bold', getSizeClass(), getColor())}>
            {formatTime(timeLeft)}
          </span>
        </div>
        {showProgress && (
        ...
        {timeLeft <= dangerThreshold && timeLeft > 0 && (
          <p className="text-xs text-red-600 mt-2 animate-pulse">Time is almost up! Submit quickly.</p>
        )}
...
export function CompactTimer({ duration }: { duration: number }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <span className={`font-mono text-sm ${timeLeft < 60 ? 'text-red-600' : 'text-gray-600'}`}>
        {format(timeLeft)}
      </span>
    </div>
  );

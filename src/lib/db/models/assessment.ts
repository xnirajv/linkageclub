...
const assessmentSchema = new Schema<IAssessment>(
  {
    ...
    questions: [{ question: { type: String, required: true }, options: { type: [String], required: true }, correctAnswer: { type: Number, required: true }, explanation: String, points: { type: Number, required: true, min: 1, default: 1 } }],
    attempts: [{ userId: { type: Schema.Types.ObjectId, ref: 'User', required: true }, score: { type: Number, required: true, default: 0 }, passed: { type: Boolean, required: true, default: false }, answers: { type: [Number], required: true, default: [] }, timeSpent: { type: Number, required: true, default: 0 }, startedAt: { type: Date, required: true, default: Date.now }, completedAt: { type: Date, default: null } }],
    badges: [{ name: { type: String, required: true }, description: String, image: String, requiredScore: { type: Number, required: true, default: 70 } }],
    prerequisites: [String],
    tags: [String],
